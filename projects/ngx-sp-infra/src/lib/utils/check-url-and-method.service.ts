...
export class CheckUrlAndMethodService {
  ...
  needsAuthRequest(url: string, method: string, urls: Map<string, string[]>): boolean {
    ...
    const lastUrlChar = urlToBeVerified.charAt(urlToBeVerified.length - 1);

    if (lastUrlChar !== "/") {
      urlToBeVerified += "/";
    }

    const urlsToVerify: IterableIterator<string> = urls.keys();

    for(const urlToVerify of urlsToVerify) {
      ...
      const regexMatch: RegExpMatchArray | null =  urlToBeVerified.match(regexString);
      ...
      if (regexMatch && regexMatch.length === 1) {
        ...
        for(let methodAux of urls.get(urlToVerify)!) {
          ...
          const regexMatchedMethod: RegExpMatchArray | null = method.match(methodAux);
