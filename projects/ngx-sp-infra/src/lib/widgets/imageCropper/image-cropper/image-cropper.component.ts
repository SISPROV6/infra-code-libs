...
export class ImageCropperComponent {
  ...
  @Input() set width(val: number | null | undefined) {
    this.imageWidth.set(val ?? null);
  }

  @Input() set height(val: number | null | undefined) {
    this.imageHeight.set(val ?? null);
  }
  ...
  fileSelected(event: any) {
    ...
    if (file) {
      const dialogRef = this.dialog.openInitialStateModal(
      ...
      instance.onClose.pipe(filter(Boolean)).subscribe(result => {
        const fileOut = new File(
          [result.blob],
          'imagem-cortada.png',
          { type: result.blob.type }
        );
        this.imageOut.emit(fileOut);
      });
    }
  }
